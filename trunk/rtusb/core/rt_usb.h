...
struct rt_urb
{
  ...
  int max_buffer_len;                     /* (in) ALL max buffer-length (needed by the hcd to create Buffers )*/
  int max_packet_size;                    /* (in) ALL max packet-size   (needed by the hcd to create Buffers )*/
  ...
  RT_SEM rt_sem;                          /* Sleep on while waiting*/
  SRTIME rt_sem_timeout;                  /* Time-Out for RT-SEMAPHORE */
  ...
  RTIME core_in_time;                     /* Send-Time */
  RTIME core_exit_time;                   /* Complete-Time */
...
/*     USB-Device locking        */
/*-------------------------------*/
extern struct usb_device *rt_usb_get_device(__u16 vendor, __u16 product ,struct usb_device *p_old_device);
...
extern struct rt_urb *nrt_usb_create_ctrl_semaphore_urb( unsigned char *name, RTIME rt_sem_timeout );
extern void nrt_usb_destroy_ctrl_semaphore_urb( struct rt_urb *p_urb );

extern struct rt_urb *nrt_usb_create_semaphore_urb( unsigned char *name, RTIME rt_sem_timeout );
...
extern int rt_usb_send_ctrl_msg(struct rt_urb *p_urb, __u8 request_type,__u8 request, __u16 wValue, __u16 wIndex, __u16 wLength , void *data);

/*--------------------------*/
/*   Asynchronous Messages  */
/*--------------------------*/
extern int rt_usb_submit_urb( struct rt_urb *p_urb );
extern int rt_usb_submit_ctrl_urb(struct rt_urb *p_urb, __u8 request_type,__u8 request, __u16 wValue, __u16 wIndex, __u16 wLength , void *data);
