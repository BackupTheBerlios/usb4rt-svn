...
nohelp_text[] = N_(
	"There is no help available for this USB4RT option.\n"),
load_config_text[] = N_(
...
load_config_help[] = N_(
	"\n"
	"For various reasons, one may wish to keep several different USB4RT\n"
	"configurations available on a single machine.\n"
	"\n"
	"If you have saved a previous configuration in a file other than the\n"
	"USB4RT's default, entering the name of the file here will allow you\n"
...
save_config_help[] = N_(
	"\n"
	"For various reasons, one may wish to keep different USB4RT\n"
	"configurations available on a single machine.\n"
	"\n"
...
int main(int ac, char **av)
{
	...
	sym = sym_lookup("USB4RT_VERSION", 0);
	sym_calc_value(sym);
	sprintf(menu_backtitle, _("USB4RT %s Configuration"),
		sym_get_string_value(sym));
	...
	do {
		cprint_init();
		cprint("--yesno");
		cprint(_("Do you wish to save your new USB4RT configuration?"));
		cprint("5");
		cprint("60");
		stat = exec_conf();
	} while (stat < 0);
	...
	if (stat == 0) {
		if (conf_write(NULL)) {
			fprintf(stderr, _("\n\n"
				"Error during writing of the USB4RT configuration.\n"
				"Your USB4RT configuration changes were NOT saved."
				"\n\n"));
			return 1;
		}
		printf(_("\n\n"
			"*** End of USB4RT configuration.\n"
			"*** Execute 'make' to build USB4RT or try 'make help'."
			"\n\n"));
	} else {
		fprintf(stderr, _("\n\n"
			"Your USB4RT configuration changes were NOT saved."
