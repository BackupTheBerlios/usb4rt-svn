 ...
 * @return Nummer des n�hsten freien Host-Controllers
 */
 ...
 * Das Bit wird in der Variablen ctrl_mask gel�cht.
...
/**
 * Sucht nach an einem Host-Controller angeschlossenen Ger�en.
 * Da die Ger�e w�rend des Ladens der Host-Controller-Driver (HCD) gesucht werden,
 * muss der HCD dem Core mitteilen, wann die Suche nach Ger�en beginnen kann.
 * Zuvor mssen vom HCD alle notwendigen Vorbereitungen getroffen sein.
 * Ein am Host-Controller ermittelter HUB wird zus�zlich in einer HUB-Liste
 * eingefgt. Diese Hubs werden ebenfalls nach angeschlossenen Ger�en untersucht.
 * @param p_hcd Zeiger auf ein struct hc_device.
 * @return -
 */
...
/**
 * Ermittelt die Sprache(n) der String-Deskriptoren eines USB-Ger�es.
 * @param p_urb Zeiger auf einen RT-URB
 * @param len L�ge des Puffers im Byte
...
/**
 * List einen String-Deskriptor aus einem USB-Ger�.
 * @param p_urb Zeiger auf einen RT-URB
 * @param string_id String-Deskriptor-ID
 * @param lang Language-ID
 * @param len L�ge des Puffers in Byte
 * @param p_buffer Zeiger auf einen Puffer
 * @return L�ge des String-Deskriptors
...
/**
 * L�cht das Feature USB_ENDPOINT_HALT eines USB-Ger�es.
 * @param p_urb Zeiger auf einen RT-URB
 * @return < 0, wenn ein Fehler auftrat
...
/**
 * L�cht und initialisiert ein USB-Ger�.
 * Vor dem Initialisieren des Ger�es werden hier noch Speicherbereiche wieder freigegeben.
 * @param p_usbdev Zeiger auf ein USB-Device (struct usb_device)
 */
 ...
 * @return NULL, wenn der Zeiger ungltig oder das Ger� schon blockiert ist.
 */
...
struct usb_device *nrt_usb_config_dev( struct hc_device *p_hcd, __u8 rh_port_nr, unsigned int lowspeed)
{
  ...
  if(ret){
    ERR("RT-USBCORE: [ERROR] while registering CTRL-URB (8 Byte), code = %d\n",ret);
 ...
 * Abh�gig vom submit_flag blockiert diese Funktion bis die Nachricht bertragen wurde
...
/**
 * Sucht in der Liste der USB-Ger�e ein bestimmtes Ger�.
 * Wenn ein zutreffendes Ger� gefunden wurde, wird dieses gleich blockiert und somit
 * vor anderen Zugriffen geschtzt.
 * @param vendor Vendor-ID des zu suchenden Ger�es.
 * @param product Product-ID des zu suchenden Ger�es.
 * @param p_old_device Zeiger auf ein USB-Ger�, ab dem gesucht werden soll.
 * @return Zeiger auf ein USB-Device, auf das die Beschreibung pa�.
 * @return NULL, wenn kein solches Ger� gefunden wurde.
 */
...
/**
 * Gibt ein zuvor mit der Funktion rt_usb_get_device blockiertes USB-Ger�e wieder frei.
 * @param p_usbdev Zeiger auf ein USB-Ger�, das freigegeben werden soll.
 */
...
/**
 * L�cht einen mit der Funktion nrt_usb_alloc_urb erstellten URB.
 ...
 * Im Gegensatz zu der Funktion nrt_usb_alloc_urb wird hier zus�zlich noch der Speicherbereich
...
/**
 * L�cht einen mit der Funktion nrt_usb_alloc_ctrl_urb erstellten Control-URB.
