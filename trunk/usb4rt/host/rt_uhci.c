...
int rt_irq_handler(struct xnintr *p_xnintr)
{
  ...
  while(p_list != &p_uhc_irq->irq_list){
    ...
    if ( !(stat & ~USBSTS_HCH) ) {                /* no State-Bit set */
      ...
      DBG(" =================== BEGIN RTAI-INTERRUPT HANDLER UHC[%d] =======================\n",p_uhcd->uhcd_nr);
...
static inline void rt_complete_urb( struct rt_privurb *p_purb )
{
  ...
  switch(usb_pipetype(p_purb->p_urb->pipe)){
    ...
    case USB_ENDPOINT_XFER_BULK:
      if( usb_pipeout(p_purb->p_urb->pipe)){
        printk("Sending Bytes: [0]: 0x%02x, [1]: 0x%02x \n",
          *(unsigned char *)p_purb->p_urb->p_transfer_buffer,
          *(unsigned char *)(p_purb->p_urb->p_transfer_buffer+1));
      }
      rt_unschedule_ctrl_bulk_urb(p_purb);
      break;
