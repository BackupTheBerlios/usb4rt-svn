...
forceconf = $(shell test -r .usb4rt_config -a -r .cfok || echo $(confgui))
endif
...
config.status: .usb4rt_config
	@test -r config.status && recf=yes || recf=no ; \
	eval `grep ^CONFIG_USB4RT_INSTALLDIR $<`; \
	eval `grep ^CONFIG_USB4RT_XENOMAIDIR $<`; \
	$(srctree)/configure \
	...
	--host=$(host_alias) \
	--with-kconfig-file=$< \
	--with-xeno-dir=$$CONFIG_USB4RT_XENOMAIDIR \
	--prefix=$$CONFIG_USB4RT_INSTALLDIR \
	$$confopts ; \
	if test $$? = 0; then \
	   touch .cfok ; \
...
help:
	@echo ; echo "This is USB4RT's build bootstrapping Makefile. In order to build USB4RT," ; \
	...
	echo '$$ make -f $$usb4rt_srcdir/makefile srctree=$$usb4rt_srcdir {xconfig,gconfig,menuconfig}' ; \
	echo '                            OR,' ; \
	echo '# Configuration using a text-based interface' ; \
	echo '$$ make -f $$usb4rt_srcdir/makefile srctree=$$usb4rt_srcdir {config,oldconfig}' ; \
...
all %::
	@if test -r GNUmakefile ; then \
	$(MAKE) -f GNUmakefile $@ ; else \
	echo "*** Please configure USB4RT first (running 'make help' in USB4RT's toplevel dir might help)." ; \
	exit 1; fi
